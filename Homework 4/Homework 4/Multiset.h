...
template <typename T> 
inline int Multiset<T>::size() const
{
    return m_size;
}

template <typename T>
inline int Multiset<T>::uniqueSize() const
{
    return m_uniqueSize;
}

template <typename T> 
inline bool Multiset<T>::empty() const
{
    return size() == 0;
}

template <typename T>
inline int Multiset<T>::erase(const T& value)
{
    return doErase(value, false);
}

template <typename T>
inline int Multiset<T>::eraseAll(const T& value)
{
    return doErase(value, true);
}

template <typename T>
inline bool Multiset<T>::contains(const T& value) const
{
